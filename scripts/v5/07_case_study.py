"""
V5 Step 07: 生成 Profile 召回案例分析。

默认做 item-to-item 相似召回；加 --query 时也会做 query-to-item profile 检索。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build V5 profile retrieval case study.")
    parser.add_argument("--config", type=str, default=str(DEFAULT_CONFIG))
    parser.add_argument("--topk", type=int, default=5)
    parser.add_argument("--num-items", type=int, default=20)
    parser.add_argument("--item-ids", type=str, default="", help="Comma-separated item ids.")
...
def main() -> None:
    ...
    profiles = {
        int(row["item_id"]): row["profile"]
        for row in read_jsonl(resolve_project_path(cfg["data"]["profiles_clean"]))
    }
    ...
    selected = (
        [int(x) for x in args.item_ids.split(",") if x.strip()]
        if args.item_ids
        else item_ids[: args.num_items].astype("int64").tolist()
    )
    cases = []
    for item_id in selected:
        if item_id not in id_to_row:
            continue
        query_vec = embeddings[id_to_row[item_id] : id_to_row[item_id] + 1].copy()
        if metric == "cosine":
        ...
        for score, neighbor_id in zip(scores[0], neighbors[0], strict=False):
            ...
            if neighbor_id == item_id or neighbor_id == -1:
                continue
            retrieved.append(
                {
                    "item_id": neighbor_id,
                    "score": float(score),
                    "title": item_meta.get(neighbor_id, {}).get("title", ""),
                }
            )
    ...
    if args.query:
        ...
        emb_cfg_path = resolve_project_path("configs/v5/profile_embedding.yaml")
        emb_cfg = load_yaml(emb_cfg_path)["embedding"]
